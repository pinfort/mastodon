...
import { AreaHeader } from '../area_header';
...
describe('<AreaHeader />', () => {
  it('renders wrapper span with correct class', () => {
    const account = makeAccount({ username: 'alice', acct: 'alice', area: 1 });
    const { container } = render(<AreaHeader account={account} />);
  ...
  it('renders the area eng name as inner class for a local account', () => {
    ...
    expect(wrapper.className).toBe('account__header__area-wrapper');
    expect(inner.className).toBe('account__header__area-kobe');
  });
  ...
  it('uses "unknown" class and "未" text for area 0 (未設定)', () => {
    ...
    expect(inner.className).toBe('account__header__area-unknown');
  ...
  it('renders instance eng name class for a remote account', () => {
    ...
    expect(inner.className).toBe('account__header__area-remote-pawoo');
